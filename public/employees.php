...
$query = isset($_GET["id"]) ? "SELECT * FROM employees WHERE id={$_GET["id"]}" : "SELECT * FROM employees"; // lee si existe un parametro 'id' que pasamos al pinchar el enlace y nos redirige a nuestra tabla donde el id es el que pasamos por parametro
$stm = $dbConnection->query($query); //$stm = statement es un objeto que nos devuelve la query a nuestra base de datos y tiene multitud de propiedad a los que podemos acceder
